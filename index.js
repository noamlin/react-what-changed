 ...
 */
const reactWhatChanged = function reactWhatChanged(dependencies, verbose = false, id) {
...
function recursiveDiff(obj, prevObj, path = '', diffs = []) {
    ...
    else { // one is pojo and the other is array
 ...
 */
const reactWhatDiff = function reactWhatDiff(obj, id) {
    if (isObj(obj)) {
        ...
        // save reference for next iteration
        globalThis[hash] = obj;
        if (this === null || this === void 0 ? void 0 : this.debug) {
            return diffs;
        }
    }
    else {
 ...
 * and print a detailed list of all property changes
 */
const whatDiff = function whatDiff(newObj, prevObj) {
    const id = '$WhatDiff$';
    const hash = `_ReactWhatDiff_${id}`;
